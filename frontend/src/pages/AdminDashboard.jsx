...
export function AdminDashboard() {
  ...
  const handleSuspendUser = async (userId, suspended) => {
    try {
      await userAPI.suspend(userId, suspended);
  ...
  const handleReviewVerification = async (requestId, status) => {
    try {
      await verificationAPI.review(requestId, status);
  ...
  const handleApproveProperty = async (propertyId, status) => {
    try {
      await propertyAPI.approve(propertyId, status);
    ...
    <div className="container mx-auto px-4 py-6" data-testid="admin-dashboard">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage users, properties, and platform operations</p>
        </div>
        ...
        <TabsList className="mb-6 flex-wrap h-auto gap-1">
          ...
          <TabsTrigger value="verification" className="gap-2" data-testid="tab-verification">
            ...
            {stats?.pending_verifications > 0 && (
              <Badge variant="destructive" className="ml-1">{stats.pending_verifications}</Badge>
            )}
          </TabsTrigger>
          <TabsTrigger value="properties" className="gap-2" data-testid="tab-properties">
            ...
            {stats?.pending_properties > 0 && (
              <Badge variant="destructive" className="ml-1">{stats.pending_properties}</Badge>
            )}
          </TabsTrigger>
            ...
            <>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold mt-1">{stats?.total_agents || 0}</p>
                    </div>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold mt-1">{stats?.total_properties || 0}</p>
                    </div>
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold mt-1">{stats?.total_inspections || 0}</p>
                    </div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <Card className="p-6 border-yellow-200 bg-yellow-50">
                  <p className="text-sm text-yellow-800">Pending Properties</p>
                  <p className="text-3xl font-bold mt-1 text-yellow-900">{stats?.pending_properties || 0}</p>
                </Card>
                <Card className="p-6 border-yellow-200 bg-yellow-50">
                  <p className="text-sm text-yellow-800">Pending Verifications</p>
                  <p className="text-3xl font-bold mt-1 text-yellow-900">{stats?.pending_verifications || 0}</p>
                </Card>
                <Card className="p-6 border-green-200 bg-green-50">
                  <p className="text-sm text-green-800">Approved Properties</p>
                  <p className="text-3xl font-bold mt-1 text-green-900">{stats?.approved_properties || 0}</p>
                </Card>
                <Card className="p-6 border-blue-200 bg-blue-50">
                  <p className="text-sm text-blue-800">Completed Inspections</p>
                  <p className="text-3xl font-bold mt-1 text-blue-900">{stats?.completed_inspections || 0}</p>
                </Card>
              </div>
                ...
                <Card className="p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{formatPrice(stats?.token_revenue || 0)}</p>
                    </div>
                  </div>
                </Card>
                <Card className="p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">Inspection Revenue</p>
                      <p className="text-2xl font-bold">{formatPrice(stats?.inspection_revenue || 0)}</p>
                    </div>
                  </div>
                </Card>
                  ...
                  {verifications.filter(v => v.status === 'pending').map((v) => (
                    <Card key={v.id} className="p-4 border-yellow-200">
                      <div className="flex items-start justify-between">
                        <div>
                          <h4 className="font-semibold">{v.user_name}</h4>
                          <p className="text-sm text-muted-foreground">{v.user_email}</p>
                          <p className="text-sm text-muted-foreground mt-1">Address: {v.address}</p>
                        </div>
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            data-testid={`approve-verification-${v.id}`}
                          >
                          ...
                          <Button
                            ...
                            data-testid={`reject-verification-${v.id}`}
                          >
                ...
                <TableBody>
                  {verifications.map((v) => (
                    <TableRow key={v.id}>
                      ...
                      <TableCell>
                        <Badge className={getStatusBadge(v.status)}>{v.status}</Badge>
                      </TableCell>
                  ...
                  {properties.filter(p => p.status === 'pending').map((p) => (
                    <Card key={p.id} className="p-4 border-yellow-200">
                      <div className="flex gap-4">
                        <img
                          src={p.images?.[0] || 'https://images.pexels.com/photos/3754595/pexels-photo-3754595.jpeg'}
                          alt=""
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-primary font-bold">{formatPrice(p.price)}/year</p>
                          <p className="text-sm text-muted-foreground">By: {p.uploaded_by_agent_name}</p>
                        </div>
                        <div className="flex flex-col gap-2">
                          <Button
                            size="sm"
                            onClick={() => handleApproveProperty(p.id, 'approved')}
                            data-testid={`approve-property-${p.id}`}
                          >
                          ...
                          <Button
                            ...
                            data-testid={`reject-property-${p.id}`}
                          >
          ...
          <Card className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {inspections.map((i) => (
                  <TableRow key={i.id}>
                    <TableCell className="font-medium">{i.property_title}</TableCell>
                    <TableCell>{i.user_name}</TableCell>
                    <TableCell>{i.agent_name || 'Unassigned'}</TableCell>
                    <TableCell>{i.inspection_date}</TableCell>
                    <TableCell>
                      <Badge className={getStatusBadge(i.status)}>{i.status}</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge className={getStatusBadge(i.payment_status)}>{i.payment_status}</Badge>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      <Dialog open={!!selectedVerification} onOpenChange={() => setSelectedVerification(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Review the agent verification documents
            </DialogDescription>
          </DialogHeader>
          {selectedVerification && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p>{selectedVerification.address}</p>
              </div>
              <div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => handleReviewVerification(selectedVerification.id, 'approved')}
            >
              Approve
            </Button>
            <Button
