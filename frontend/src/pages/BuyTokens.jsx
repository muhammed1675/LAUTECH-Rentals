    ...
    try {
      ...
      if (window.Korapay) {
        window.Korapay.initialize({
          ...
          onSuccess: (data) => {
            toast.success('Payment successful! Confirming tokens...');
            navigate(`/profile?reference=${data.reference}`);
          },
          onFailed: () => {
        ...
        // Fallback redirect
        window.location.href = `https://checkout.korapay.com/checkout?amount=${amount}&currency=NGN&reference=${reference}&merchant=${publicKey}&email=${encodeURIComponent(email)}`;
      }
    } catch (error) {
